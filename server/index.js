...
io.on('connection', (socket) => {
  ...
  socket.on('generate code', async (collaborationID) => {
    // Simulação de chamada à biblioteca fictícia AIFramework para geração de código
    const generatedCode = await AIFramework.generateCode(collaborations[collaborationID].code.getValue());
    collaborations[collaborationID].code.setValue(generatedCode);
    io.to(collaborationID).emit('editor content', generatedCode);
  });
